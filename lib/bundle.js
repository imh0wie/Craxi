...
class Commercial extends _building__WEBPACK_IMPORTED_MODULE_0__["default"] {
    constructor(options, game) {
        super(options, game); //, option[prev], option[next]); // 7.5, _ , 110, 40
...
class Game {
  ...
  constructor(ctx) {
    this.ctx = ctx;
    // debugger
    this.background = new _background__WEBPACK_IMPORTED_MODULE_0__["default"](ctx, this);
    // debugger
...
class Sidewalk {
    ...
    draw(ctx, side) {
        let buildings = _util__WEBPACK_IMPORTED_MODULE_0__["generateBuildings"](side, this.game);
        debugger
...
const generateBuildings = (side, game) => {
    let buildings = [];
    ...
    for (let i = 0; i < 9; i++) {
        ...
        let commercial = new _building_commercial__WEBPACK_IMPORTED_MODULE_0__["default"](options, game);
        // debugger
